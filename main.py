...
async def listStat(ctx):
  ...
  for k in response:
    ...
    try:
      ...
      total = total -1
  await ctx.channel.send(f'{complete} / {total} or {int(100*(complete/total))}% have completed the challenge so far!!')
...
async def auth(ctx):
  # intro message
    ...
    try:
      ...
      try:
        ...
        await c.send(f"{ctx.message.author.mention}  your privacy settings are preventing the bot messaging you \n Error please contact a Server admin")
  ...
  # Step 2 - Send validation code
    ...
    body = 'Hi '+ str(ctx.author)+' please copy and paste the following code into the discord private chat\n\n'+authCode
